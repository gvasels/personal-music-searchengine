...
**CRITICAL: Check which routing pattern the project uses before creating new pages.**

### Code-Based Routing (This Project)
Routes are manually defined in `frontend/src/main.tsx`:
```typescript
// 1. Import the page component
import NewPage from './routes/path/to/page';

// 2. Create the route
const newRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: '/path/to/page',
  component: withAuthGuard(NewPage),  // Use withAuthGuard for protected routes
});
...
Routes auto-discovered from file structure - check if `routeTree.gen.ts` contains all routes.
If using file-based routing, page files need `export const Route = createFileRoute(...)`.

### How to Identify
1. Check `frontend/src/main.tsx` - if routes are imported and registered there, it's code-based
2. Check `frontend/src/routeTree.gen.ts` - if it only has a few routes, likely code-based
3. Check existing page files - if they use `createFileRoute`, it's file-based
